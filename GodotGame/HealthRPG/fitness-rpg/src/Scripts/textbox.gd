...
func _ready() -> void:
	...
	#start("System", ["Hello World!", "This is a test of the textbox system, for maxing out this."])
...
func set_custom_font(font_path: String, size_override: int = 0) -> void:
	if FileAccess.file_exists(font_path):
		var new_font = load(font_path)
		if dialogue.label_settings:
			dialogue.label_settings.font = new_font
			if size_override > 0:
				dialogue.label_settings.font_size = size_override
		else:
			dialogue.add_theme_font_override("font", new_font)
			if size_override > 0:
				dialogue.add_theme_font_size_override("font_size", size_override)
				# "font" is the theme property name for the main font
	else:
		print("Error: Font file not found at ", font_path)

func reset_font() -> void:
	# Removes the override so it goes back to the default theme
	dialogue.remove_theme_font_override("font")
	dialogue.remove_theme_font_size_override("font_size")
