...
@onready var live_cam: TextureRect = $"."
...
func _ready():
	...
	var cam_tex_y = live_cam.material.get_shader_parameter("camera_y")
	var cam_tex_CbCr = live_cam.material.get_shader_parameter("camera_CbCr")
	
	cam_tex_y.camera_feed_id = camera.get_id()
	cam_tex_CbCr.camera_feed_id = camera.get_id()
	
	live_cam.material.set_shader_parameter("camera_y", cam_tex_y)
	live_cam.material.set_shader_parameter("camera_CbCr", cam_tex_CbCr)
